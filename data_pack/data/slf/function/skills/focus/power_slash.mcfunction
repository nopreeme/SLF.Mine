execute if entity @s[tag=slf.skill.power_slash] run tellraw @s {"text":"You already know this skill!","color":"red"}
execute if entity @s[tag=slf.skill.power_slash] run return fail
execute if score @s slf.focus matches 1 run tellraw @s {"text":"Already focusing this skill.","color":"yellow"}
execute if score @s slf.focus matches 1 run return fail
