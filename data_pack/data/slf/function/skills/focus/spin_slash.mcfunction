execute if entity @s[tag=slf.skill.spin_slash] run tellraw @s {"text":"You already know this skill!","color":"red"}
execute if entity @s[tag=slf.skill.spin_slash] run return fail
execute if score @s slf.focus matches 6 run tellraw @s {"text":"Already focusing this skill.","color":"yellow"}
execute if score @s slf.focus matches 6 run return fail
